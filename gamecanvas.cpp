...
void GameCanvas::paintEvent(QPaintEvent *)
{
    ...
    {
        painter.drawImage((int)unit->getX(), ROAD_Y - unit->getHeight() - (int)unit->getY(), *unit->getImage(), unit->getCurrentFrame() * unit->frameSize.width(), 0, unit->getWidth(), unit->getHeight(), 0);
    }
}
